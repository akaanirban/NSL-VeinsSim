...
class BaseWaveApplLayer : public BaseApplLayer {
    ...
    protected:
        ...
        virtual void populateWSM(WaveShortMessage*  wsm, int rcvId=0, int serial=0, const char * wsmData=NULL);  //taylor code, added opp_string wsmData=null

        /** @brief this function is called upon receiving a WaveShortMessage */
        virtual void onWSM(WaveShortMessage* wsm) { };

        /** @brief this function is called when a custom WSM is received, taylor code*/
        virtual void onCWSM(WaveShortMessage* wsm) { };
        ...
        /**
         * @brief helper function for writing individual node message list
         *
         * @param no param
         */
        ...
        /**
         * @brief helper function for writing RSU Message
         *
         * @param wsm the message in which RSU data is to be stored
         */
        virtual void prepareRSUmessage(WaveShortMessage* wsm);
    ...
    protected:
        ...
        simtime_t RSUMsgInterval; //anirban code
        bool sendBeacons;
        bool sendRSUMsgs;
        int numRSUs;//anirban code
        int numHotSpots; //anirban code
        bool hotspotFlag; //anirban code
        ...
        uint32_t generatedCWSMs;  //taylor code
        uint32_t receivedWSMs;
        uint32_t receivedWSAs;
        uint32_t receivedBSMs;
        uint32_t receivedCWSMs;  //taylor code
        vector<string> MessageList; //anirban code

        /* messages for periodic events such as beacon and WSA transmissions */
        cMessage* sendBeaconEvt;
        cMessage* sendWSAEvt;
        WaveShortMessage* sendRSUCSWM; //anirban code
};
