...
void BaseWaveApplLayer::initialize(int stage) {
    ...
    if (stage==0) {
        ...
        RSUMsgInterval = par("RSUMsgInterval"); //anirban code
        sendRSUMsgs = par("sendRSUMsgs"); //anirban code
        numRSUs = par("numRSUs"); //number of RSUs //anirban code
        numHotSpots = par("numHotSpots"); //number of hotspots //anirban code
        hotspotFlag = par("hotspotFlag"); //whether this is a hotspot or not
        ...
        sendRSUCSWM = new WaveShortMessage(); //initialise the RSU CSWM //anirban code
        //prepareRSUmessage(sendRSUCSWM); //prepare the SEND_RSU_EVT //anirban code
        generatedBSMs = 0;
        generatedWSAs = 0;
        generatedWSMs = 0;
        generatedCWSMs = 0;
        ...
        receivedWSAs = 0;
        receivedWSMs = 0;
        receivedCWSMs = 0;  //Updated sim code
    }
    else if (stage == 1) {
...
void BaseWaveApplLayer::populateWSM(WaveShortMessage* wsm, int rcvId, int serial, const char * wsmData) {  //Updated sim code, const char * wsmData
    ...
    wsm->setWsmData(wsmData);  //Updated sim code, setting the data
...
void BaseWaveApplLayer::handleLowerMsg(cMessage* msg) {
    ...
    ASSERT(wsm);
    if (wsm->getPsid() == -10) {  //Updated sim code, message handling for custom message
        receivedCWSMs++;
        onCWSM(wsm);
    }
    else if (BasicSafetyMessage* bsm = dynamic_cast<BasicSafetyMessage*>(wsm)) {
        cout<<getParentModule()->getName()<<endl;
        receivedBSMs++;
        onBSM(bsm);
    }
    else if (WaveServiceAdvertisment* wsa = dynamic_cast<WaveServiceAdvertisment*>(wsm)) {
...
void BaseWaveApplLayer::handleSelfMsg(cMessage* msg) {
    ...
    case SEND_BEACON_EVT: {
        printf(" RSU INdex : %d\n", getParentModule()->getIndex()); //updated code, anirban
    ...
    case SEND_RSU_EVT:   { //anirban code
        ...
        sendDown(wsmRSU);
        scheduleAt(simTime() + RSUMsgInterval, wsmRSU);
        break;
    }
    default: {
...
void BaseWaveApplLayer::finish() {
    ...
    recordScalar("generatedCWSMs", generatedCWSMs);  //Updated sim code, analysis recording
    recordScalar("receivedCWSMs", receivedCWSMs);  //Updated sim code, analysis recording

    string parentName = getParentModule()->getName();
    if (!(parentName.find("rsu") != string::npos)){
        writeMessages(); //anirban code
    }
}
...
BaseWaveApplLayer::~BaseWaveApplLayer() {
    ...
    cancelAndDelete(sendRSUCSWM); //anirban code
...
void BaseWaveApplLayer::checkAndTrackPacket(cMessage* msg) {
    ...
    if (dynamic_cast<BasicSafetyMessage*>(msg)) {
        ...
        WaveShortMessage* wsm = dynamic_cast<WaveShortMessage*>(msg);  //Updated sim code, used for packet tracking
        ASSERT(wsm);

        if (wsm->getPsid() == -10) {
            DBG_APP << "sending down a CWSM" << std::endl;
            generatedCWSMs++;
        }
        else {
...
void BaseWaveApplLayer::prepareRSUmessage(WaveShortMessage* wsm){  //anirban code
    std::vector<int> msgTag(numRSUs);
    msgTag[getParentModule()->getIndex()] =1; //set the tag 1 value here
    std::vector<int>::const_iterator i;
    std::string msgTagStr = "";
    for(i=msgTag.begin(); i!=msgTag.end(); ++i){
           msgTagStr+=std::to_string(*i);
        }
...
void BaseWaveApplLayer::writeMessages(){  //anirban code
    mobility = TraCIMobilityAccess().get(getParentModule());
    std::ostringstream filenameStream;
    filenameStream << "results/messages_" << mobility->getExternalId() << ".txt";
    std::string filename = filenameStream.str();
    ofstream newfile (filename);
    cout<< "Car : "<<mobility->getExternalId()<<" has aggregated "<<MessageList.size()<<" messsages."<<endl;
