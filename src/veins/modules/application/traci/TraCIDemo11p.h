...
class TraCIDemo11p : public BaseWaveApplLayer {
	...
	protected:
		...
		WaveShortMessage* aggregatedMessage;
	protected:
        virtual void onWSM(WaveShortMessage* wsm);
        virtual void onWSA(WaveServiceAdvertisment* wsa);
        void onCWSM(WaveShortMessage* wsm);

        virtual void writeMessages();
        virtual void handleSelfMsg(cMessage* msg);
		virtual void handlePositionUpdate(cObject* obj);
		virtual void aggregateMessage(WaveShortMessage* newmsg);
		virtual void redundancyAvoidance(WaveShortMessage* aggmsg, string newmsg);
};
